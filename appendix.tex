...
CKA-only reward decreases from 0.354 (peak at step 100) to 0.308 (step 300), yet accuracy does not improve, directly confirming reward-accuracy decoupling at this training horizon.
...
The $\alpha$ dose-response effect is not statistically significant on MATH-500: SFT accuracy varies by ${\sim}$6\,pp across $\alpha$ values, but with stderr ${\approx}$2\,pp per point, attributable to high truncation rates at the 2048-token limit and floor effects at this difficulty level.
...
We select $\varepsilon{=}2$\,pp because it substantially exceeds the observed cross-seed standard deviation ($\sigma{=}0.19$\,pp for CKA-only, $0.38$\,pp for Standard at 20\% sparsity), ensuring the margin reflects meaningful practical differences rather than measurement noise.
