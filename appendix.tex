...
See Figure~\ref{fig:extended_training} in the main text for the 300-step extended training results.
At 300 steps, GSM8K accuracy: CKA-only 64.37\% (identical to 100-step seed 42), Standard 59.29\% (within 100-step seed variance of $\pm$0.38\,pp).
The unchanged accuracy despite rising CKA-only reward (0.308 at step 300 vs.\ 0.354 peak at step 100) directly demonstrates reward-accuracy decoupling at this training horizon.

\section{MATH-500 Out-of-Distribution Evaluation}
\label{sec:appendix_math500}

\input{tables/tab_math500}
...
This is consistent with MATH-500's higher difficulty compressing all pruned methods toward a capability floor, reducing inter-method differentiation (\S\ref{sec:ood}).
...
\input{tables/tab_cka_stability}
...
All tested methods collapse at 30\% sparsity: Standard retains 4\% accuracy, CKA-only 1.6\%, and \rasp{} 0\%.
...
\input{tables/tab5_cross_scale}

On DeepSeek-R1-Distill-Qwen-1.5B at 20\% sparsity, the three-method ordering is preserved (CKA-only 35.48\% $\gg$ \rasp{} 26.16\% $>$ Standard 22.29\%), with reward-accuracy decoupling confirmed at this scale ($|\Delta| < 1$\,pp: CKA-only $-$0.15, Standard $-$0.61\,pp).
...
As a sanity check, we test whether CKA-only's advantage reflects mere layer-depth bias by constructing a depth-first baseline: synthetic importance scores force removal of late layers first (L18--27: 0.001, L10--17: 0.5, L0--9: 1.0), then standard pruning at 20\% sparsity.
...
\centering
\small
\caption{TOST equivalence results ($\varepsilon{=}2$\,pp). All observed $|\Delta|{<}1$\,pp.}
\label{tab:tost}
\begin{tabular}{@{}llrr@{}}
\toprule
Criterion & GRPO seed & $\Delta$ (pp) & $p_\text{TOST}$ \\
\midrule
CKA-only & seed 42 & $-$0.45 & 0.008 \\
CKA-only & seed 123 & $-$0.30 & 0.005 \\
CKA-only & verify & $-$0.15 & 0.001 \\
CKA-only & seed 456 & $-$0.68 & 0.023 \\
\midrule
Standard & seed 123 & $-$0.68 & 0.020 \\
Standard & seed 456 & $-$0.99 & 0.069 \\
Standard & fair\_v2 & $-$0.23 & 0.010 \\
Standard & ext 300 & $-$0.08 & 0.007 \\
\bottomrule
\end{tabular}
\end{table}
