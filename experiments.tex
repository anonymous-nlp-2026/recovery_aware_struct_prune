...
Decoupling is not universal: at 15\% sparsity, GRPO amplifies the CKA-only advantage (SFT gap 5.69\,pp $\to$ GRPO gap 7.20\,pp), but at 20--25\% $|\Delta|$ remains below 1\,pp without exception (Tables~\ref{tab:decoupling},~\ref{tab:cross_sparsity}; Appendix~\ref{sec:appendix_cross_sparsity}), and dose-response $\alpha$ variations stay within $1.14$\,pp (Table~\ref{tab:alpha}; Appendix~\ref{sec:appendix_math500}).

Small $|\Delta|$ alone does not establish equivalence; it could reflect insufficient power. Two One-Sided Tests (TOST; $\varepsilon{=}2$\,pp ${\approx}4\times$ cross-seed $\sigma$; $N{=}1{,}319$ paired examples) confirm statistical equivalence for 7/8 comparisons ($p{<}0.05$; Appendix~\ref{sec:appendix_tost}), robust across seeds (0.37\,pp range; Appendix~\ref{sec:appendix_multiseed}) and extended training (300 steps; \S\ref{sec:extended_analysis}).
...
Mechanistically, removing high-overlap structures forces the model to redistribute computation across functionally diverse pathways, creating richer gradient landscapes: diverse structures produce varied responses to the same prompt, generating the reward variance that GRPO requires for policy improvement.
...
This indicates that the capacity for reward-driven learning is determined at pruning time, not during recovery: importance-preserving pruning creates a structurally homogeneous model that cannot generate the response diversity GRPO needs, and additional training cannot compensate for this missing structural prerequisite.
